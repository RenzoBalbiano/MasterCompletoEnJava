...
public class SentenciaForArreglo {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(" ");
        for (int i = 0; i < count; i++) {
            if (nombres[i].equalsIgnoreCase("rEnZo") || nombres[i].equalsIgnoreCase("ceCiliA")) {
                continue;
            }
            System.out.println(i + ".- " + nombres[i]);
        }
        ...
        for (int i = 0; i < count; i++) { // i debe ser menor a count. Entonces entra, si es 0 no es mayor a count, no
                                          // se cumple la condición y no entra.
